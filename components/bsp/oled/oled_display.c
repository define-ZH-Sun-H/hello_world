 ...
 *
 * 调用一次完成两件事：
 *   1. 创建 disp_queue（长度 5），供各任务发送 DISP_MSG_xxx 消息
 *   2. 创建 display_task，绑定 core 1、优先级 5
 *
 * 初始化时置 dirty = true，确保首帧立即渲染。
 */
void oled_display_task_start(void)
{
    ...
    /*
     * 创建显示任务：
     *   栈 3072 bytes、优先级 5、绑定 core 1
     *   优先级 5 高于 key_task(6) 之外的传感器任务，
     *   确保 UI 响应流畅。
     */
